...
void HandlerApp::onTangoEventAvailable(const TangoEvent* event) {
  ...
  LOGI("Test accesseur : %g", getTest());
}


void HandlerApp::onFrameAvailable(TangoCameraId id, const TangoImageBuffer* buffer) {
  //std::lock_guard<std::mutex> lock(picture_mutex_);
  LOGI("OnFrameAvailable(): Cam frame data received id :%d || camera color id : %d", id, TANGO_CAMERA_COLOR);

  if(id == TANGO_CAMERA_COLOR){//0 is the id of the CAMERA_COLOR, and the TangoCameraId id is given in hexa
    LOGI("Test accesseur : %g", HandlerApp::getTest());
    LOGI("width = %d", buffer->width);
    LOGI("height  = %d", buffer->height);
    LOGI("cur_frame_timstamp  = %g", buffer->timestamp);
    //LOGI("frame_number  = %d", buffer->frame_number);
    LOGI("format = %d", buffer->format);
    LOGI("data_size = %d", buffer->width*buffer->height);
    LOGI("timeStamp : %g", buffer->timestamp);
    LOGI("------------------------------------------");
  }
}

double HandlerApp::getTest(){
  return test_;
}
