...
bool HandlerApp::OnTangoServiceConnected(JNIEnv* env, jobject binder) {
  ...
  if (ret != TANGO_SUCCESS) {
    LOGE(
        "HandlerApp: Failed to set Binder Tango service with"
        "error code: %d",
        ret);
    return false;
  }
...
int HandlerApp::TangoSetupConfig() {
  ...
  if (ret != TANGO_SUCCESS) {
    LOGE(
        "HandlerApp: config_enable_auto_recovery() failed with error"
        "code: %d",
        ret);
    return ret;
  }
  ...
  if (ret != TANGO_SUCCESS) {
    LOGE(
        "HandlerApp: config_enable_depth() failed with error"
        "code: %d",
        ret);
    return ret;
  }
  ...
  /*
  ------------------------------------------------------------------------------------------------
  function part added by bastien to allow TangoService_connectOnFrameAvailable()
  ------------------------------------------------------------------------------------------------
  */
  /*
  // Enable color camera from confi
  ...
  bool testConfig;
  TangoConfig_getBool(tango_config_, "config_enable_color_camera", &testConfig);
  picture_data_.SetPictureDebugString("config_enable_color_camera done", ret, testConfig);
...
int HandlerApp::TangoConnectCallbacks() {
  ...
  if (ret != TANGO_SUCCESS) {
    LOGE(
        "HandlerApp: Failed to connect to point cloud callback with error"
        "code: %d",
        ret);
    return ret;
  }
  ...
  if (ret != TANGO_SUCCESS) {
    LOGE(
        "HandlerApp: Failed to connect to pose callback with error"
        "code: %d",
        ret);
    return ret;
  }
  ...
  /*
  -------------------------------------------------------------------------------------------------
  function part added by bastien
  -------------------------------------------------------------------------------------------------
  */
  /*
  // Attach the onFrameAvailable callback.
  // The callback will be called after the service is connected.
  ret = TangoService_connectOnFrameAvailable(TANGO_CAMERA_COLOR, NULL,onFrameAvailableRouter);
  if (ret != TANGO_SUCCESS) {
...
*/
  ...
  bool HandlerApp::TangoConnect() {
    ...
    if (err != TANGO_SUCCESS) {
      LOGE(
          "HandlerApp: Failed to connect to the Tango service with"
          "error code: %d",
          err);
      return false;
    }
