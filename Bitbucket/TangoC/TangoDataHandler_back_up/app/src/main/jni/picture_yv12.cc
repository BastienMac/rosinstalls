...
std::string PictureDataYV12::GetPictureDebugString() { return picture_string_; }

void PictureDataYV12::SetPictureDebugString(std::string s, int errorInt , bool errorBool){
  std::stringstream string_stream;
  string_stream.setf(std::ios_base::fixed, std::ios_base::floatfield);
  string_stream.precision(3);
  string_stream << s << " ==>" << errorInt << " // bit = " << errorBool;
